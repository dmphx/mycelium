...
- Productie draait op de P920 Unraid-host; beheer via `ssh unraid` en de private Onyx compose
...
| Component | Waarde |
|---|---|
| Runtime-host | Lenovo P920, Unraid, bereikbaar via `ssh unraid` |
| Mycelium URL | Private Onyx-route; runtime in container `mycelium` |
| Projectmap host | `/mnt/user/appdata/onyx/`, compose `15b-media-automation.yml` |
...
| Branch | `hardened` voor Onyx-productie; `main` volgt upstream |
...
De private Onyx Plex compose kopieert het wrapper-script via deployment-specifieke mounts:
```yaml
entrypoint:
  - /bin/sh
  - -c
  - |
    ...
    exec /init
volumes:
  - /mnt/user/appdata/mycelium/spore:/spore
  - /mnt/user/appdata/mycelium/data/plex-media:/plex-media:ro
```
...
ssh unraid "cd /mnt/user/appdata/onyx/compose && docker compose -f 15-media-servers.yml up -d plex"
```
...
```bash
# Logs live op de P920
ssh unraid "docker logs -f mycelium"

# Herstarten na codewijziging
ssh unraid "cd /mnt/user/appdata/onyx/compose && docker compose --env-file ../.env -f 15b-media-automation.yml restart mycelium"

# Rebuild na Dockerfile wijziging
ssh unraid "cd /mnt/user/appdata/onyx/compose && docker compose --env-file ../.env -f 15b-media-automation.yml up -d mycelium"
